...
async function runSlot(slot) {
  while (!stopping) {
    ...
    console.log(JSON.stringify({level:"info",event:"job_claimed",jobID:job.id,kind:job.kind,attempt:job.attempt_count,slot}));
    ...
    try {
      ...
      console.log(JSON.stringify({level:"info",event:"job_completed",jobID:job.id,kind:job.kind,latencyMs:output.latencyMs,model:output.model}));
    } catch (error) {
      ...
      console.error(JSON.stringify({level:"error",event:retry?"job_retrying":"job_failed",jobID:job.id,kind:job.kind,attempt:job.attempt_count,errorCode:transient?"transient":"workflow_error"}));
