export function catalogPrompt(item) {
  return [
    ...
    "Use a portrait 4:5 transparent canvas with a true alpha channel. Center the garment and make it fill roughly 80% of the canvas while keeping every edge fully visible. Transparency must continue through every opening in lace, crochet, mesh, eyelets, straps, and cutwork.",
    ...
    "Output exactly one tightly cropped catalog cutout with a transparent background, no green screen, checkerboard, or pixel grid, and no backdrop color, caption, or surrounding scene."
  ].join(" ");
}
