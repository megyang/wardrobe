...
async function style(body, signal = null, workerIndex = null) {
  ...
  const prompt = [
    ...
    "Treat owned scarves as real styling pieces, not background metadata. Consider a scarf when its actual pictured color, print, scale, length, and drape strengthen a specific inspiration translation or bring the owned garments together, but do not force one into a look that is already complete. When a scarf is included, the rationale must name one physically plausible way to wear that exact scarf with that outfit (for example narrow neck tie, loose drape, choker with trailing ends, headscarf, hair tie, belt, or bag tie). Use the attached laid-flat and worn views to judge what is feasible, and vary the treatment across candidates rather than repeating one method.",
  ...
  const criticPrompt = [
    ...
    "If a selected outfit contains an owned scarf, its final rationale must explicitly say how to wear it in that outfit. Confirm the treatment is plausible from the scarf's attached laid-flat and worn views, and prefer different treatments when more than one selected look contains a scarf.",
