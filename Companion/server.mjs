...
async function materializeCandidateBoards(candidates, wardrobe, garmentVisuals, imageOffset = 0, legendLabel = "assembled candidate") {
  ...
  // The client sends the clean catalog cutout first, followed by worn/detail
  // views of that same garment for visual reasoning.
  for (const item of garmentVisuals || []) if (!byVisualID.has(item.id)) byVisualID.set(item.id, item.imageBase64);
...
async function analyze(body, reportProgress = async () => {}, signal = null, workerIndex = null) {
  ...
  try {
    ...
    const result = await structured(
      `${inventoryInstruction} Exclude the person and background. Include a purse, scarf, or piece of jewelry only when it is deliberately shown, worn clearly enough to identify as a distinct item, or is a prominent part of the outfit; ignore tiny, occluded, or ambiguous accessories. Describe only visible evidence and explicitly list unknown details. Never invent logos, text, pockets, trim, fasteners, hardware, gemstones, materials, or construction. Give each item a conservative fingerprint from visible color, material, silhouette, and distinctive marks. Choose exactly one matching subcategory: tops use long_sleeve, tank_top, t_shirt, sleeveless, or blouse; bottoms use shorts, mini_skirt, midi_skirt, maxi_skirt, or pants; outerwear uses coverup, sweater, jacket, or coat; accessories use tights, hat, purse, jewelry, scarf, or misc. Use scarf for neck scarves, skinny scarves, silk squares, wraps, and similar wearable fabric accessories. Use purse for handbags, totes, clutches, crossbody bags, shoulder bags, and small fashion backpacks. Use jewelry for necklaces, earrings, bracelets, rings, brooches, and anklets. Dresses and shoes use none. Use mini_skirt for hems above the knee, midi_skirt for hems from around the knee through mid-calf, and maxi_skirt for ankle- or floor-length skirts. Use none when the visible evidence does not establish a skirt's length. Prefer blouse for a visibly blouse-like woven or dress top, tank_top for a tank silhouette, t_shirt for a tee, sleeveless for another sleeveless top, and long_sleeve for another long-sleeved top.`,
...
async function style(body, signal = null, workerIndex = null) {
  ...
  const prompt = [
    ...
    "Treat owned scarves as real styling pieces, not background metadata. Consider a scarf when its actual pictured color, print, scale, length, and drape strengthen a specific inspiration translation or bring the owned garments together, but do not force one into a look that is already complete. The uploaded worn view is evidence of dimensions and drape, not the styling answer: do not merely repeat or describe the shape shown in that photo. When a scarf is included, end the rationale with two actionable, physically different treatments labeled 'How to wear the scarf:' and 'Another way:'. Tailor both to that exact scarf and outfit—for example a narrow neck tie versus choker with trailing ends, headband, ponytail ribbon, soft belt, or bag-handle tie. Use the laid-flat and worn views to rule out infeasible treatments, and vary the primary treatment across candidates.",
  ...
  const criticPrompt = [
    ...
    "If a selected outfit contains an owned scarf, its final rationale must preserve two actionable alternatives labeled 'How to wear the scarf:' and 'Another way:'. The uploaded worn pose is evidence only; neither instruction may merely describe or repeat the photographed shape. Confirm both transformations are plausible from the scarf's laid-flat and worn views, suit the selected outfit, and prefer different primary treatments when more than one selected look contains a scarf.",
...
async function recommendItem(body, signal = null, workerIndex = null) {
  ...
  const prompt = [
    ...
    "Treat an eligible scarf as a real accessory when its pictured color, print, scale, and drape improve this exact collage. If you select a scarf, name one physically plausible way to wear it in the rationale; do not select two scarves.",
