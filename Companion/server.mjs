...
async function style(body, signal = null, workerIndex = null) {
  ...
  const prompt = [
    ...
    "Treat owned scarves as real styling pieces, not background metadata. Consider one only when its pictured color, print, scale, length, and drape strengthen the outfit. The worn view is evidence, not the styling answer: never copy or describe its photographed shape. For a scarf outfit, write one short sentence about why the outfit works, then two physically different treatments labeled 'Scarf:' and 'Alternative:'. Each treatment must be 12 words or fewer and tailored to that scarf and outfit. Use the visual references to rule out infeasible treatments and vary the primary treatment across candidates.",
  ...
  const criticPrompt = [
    ...
    "For a scarf outfit, preserve one brief reason plus two actionable treatments labeled 'Scarf:' and 'Alternative:'. Keep each treatment to 12 words or fewer. Never repeat the photographed pose; confirm both transformations suit the outfit and are physically plausible from the scarf views.",
