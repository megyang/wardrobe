...
export function recommendationTarget(value, wardrobe = []) {
  ...
  if (subcategory && !SUBCATEGORIES[category].includes(subcategory) &&
      !wardrobe.some(item => item?.category === category && item?.subcategory === subcategory)) {
    throw new Error("Choose a subcategory within the selected category.");
  }
  return { category, subcategory };
}

export function eligibleRecommendationItems(wardrobe, selectedIDs, targetValue) {
  const target = recommendationTarget(targetValue, wardrobe);
