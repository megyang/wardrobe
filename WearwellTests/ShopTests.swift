...
final class ShopTests: XCTestCase {
    ...
    func testExistingShoppingProfilesReceiveNewRetailerDefaultsOnce() throws {
        ...
        XCTAssertEqual(profile.selectedPriceTier, "mid")
        XCTAssertEqual(profile.qualityPriority ?? 0.65, 0.65)
        XCTAssertTrue(profile.materialPriorities.isEmpty)
        XCTAssertTrue(ShoppingRetailer.applyBundledUpdates(to: &profile))
        XCTAssertEqual(profile.materialPriorities, ["cotton"])
        XCTAssertEqual(profile.qualityPriority, 0.8)
